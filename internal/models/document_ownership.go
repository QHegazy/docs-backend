...
func (d *DocumentOwnership) Delete(pool *pgxpool.Pool, resultChan chan<- ResultChan[error]) {
	query := `DELETE FROM public.document_ownerships WHERE document_id = $1`
	_, err := pool.Exec(context.Background(), query, d.DocumentID)
	resultChan <- ResultChan[error]{Error: err}
}

func (d *DocumentOwnership) Query(pool *pgxpool.Pool, resultChan chan<- ResultChan[*DocumentOwnership]) {
	var doc DocumentOwnership
	query := `SELECT user_id, document_id FROM public.document_ownerships WHERE document_id = $1`
	err := pool.QueryRow(context.Background(), query, d.DocumentID).Scan(&doc.UserID, &doc.DocumentID)
	if err != nil {
